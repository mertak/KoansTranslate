...
module EdgeCase
  ...
  class Sensei
    ...
    def observe(step)
      if step.passed?
        ...
        if @pass_count > progress.last.to_i
          @observations << Color.green("#{step.koan_file}##{step.name} sende yeni ufuklar açtı.")
        end
      else
        @failed_test = step
    ...
    def show_progress
      ...
      print Color.green("önünde uzuun bir yol var [")
...
ENDTEXT
    ...
    def encourage
      ...
      if ((recents = progress.last(5)) && recents.size == 5 && recents.uniq.size == 1)
        ...
        puts Color.cyan("  İlerleme kaydediyorsun. Muhteşem! #{progress.last}. seviye tamamlandı.")
      end
    end

    def guide_through_error
      puts
      puts "Aradığın cevap bu olmalıydı:"
      puts Color.red(indent(failure.message).join)
      puts
      puts "Aşağıdaki kod üzerinde çalışmaya devam et:"
      if assert_failed?
    ...
    def a_zenlike_statement
      if !failed?
        zen_statement =  "Aşılamayacak dağ yoktur."
      else
        zen_statement = case (@pass_count % 10)
        when 0
          "Aşılamayacak dağ yoktur."
        when 1, 2
          "kuralları öğren bu sayede kural çiğnemeyi öğrenebilirsin"
        when 3, 4
          "söz gümüşse sükut altındır atasözünü hatırla"
        when 5, 6
          "uyku en iyi meditasyondur"
        when 7, 8
          "başarısız olduğunda ders çıkarmayı öğren"
        else
          "Göründükleri gibi değiller, aksine tam tersiler."
        end
      end
        ...
        args.each do |arg|
          ...
          when /^-n(.*)$/
            ...
            if File.exist?(arg)
              ...
              fail "Bilinmeyen komut satırı argümanı '#{arg}'"
            end
          end
        end
      end
